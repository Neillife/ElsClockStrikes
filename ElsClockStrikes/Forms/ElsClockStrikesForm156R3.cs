...
namespace ElsClockStrikes
{
    public partial class ElsClockStrikesForm
    {
        ...
        private void ProcessWindowsSetting156R3(bool isBackToOriginCheck)
        {
            ...
            重置計時器156R3Label.Location = isBackToOriginCheck ? 重置計時器LabelOriginPos : new Point(大黑Label.Left + 大黑Label.Width - 重置計時器156R3Label.Width + 10, 重置計時器156R3按鍵Label.Top + 重置計時器156R3按鍵Label.Height - 重置計時器156R3Label.Height - 3); ;
            ...
            大黑按鍵Label.Text = 大黑ComboBox.Text;
            陰陽陣按鍵Label.Text = 陰陽陣ComboBox.Text;
            三連按鍵Label.Text = 三連ComboBox.Text;
            R3156控場按鍵Label.Text = R3156控場ComboBox.Text;
            重置計時器156R3按鍵Label.Text = 重置計時器156R3ComboBox.Text;
        ...
        private void WindowsSetting156R3_Click(object sender, EventArgs e)
        {
            ...
            {
                WindowsSetting156R3.BaseColor = Color.FromArgb(65, 105, 225);
                WindowsSetting156R3.OnHoverBaseColor = Color.FromArgb(45, 85, 205);
                WindowsSetting156R3.Text = "調整設定";
        ...
        {
            大黑設定音效Button.Visible = !大黑設定音效Button.Visible;
            陰陽陣設定音效Button.Visible = !陰陽陣設定音效Button.Visible;
            三連設定音效Button.Visible = !三連設定音效Button.Visible;
            R3156控場設定音效Button.Visible = !R3156控場設定音效Button.Visible;
            this.LoadDefaultSound156R3();
        }
